...
class FixedDurationEventReaderHDF5:
    ...
    **Note**: This reader is much slower than the FixedSizeEventReader.
    ...
    def __init__(self, events, duration_ms=20.0, start_ts=None):
        self.events = events

        self.last_stamp = start_ts
        self.duration_s = duration_ms / 1000.0
        self.last_idx = 0
    ...
    def __next__(self):
        with Timer('Reading event window from file'):
            event_list = []
            for idx, (xi, yi, ti, pi) in enumerate(zip(self.events['x'][self.last_idx:], 
                                                self.events['y'][self.last_idx:], 
                                                self.events['t'][self.last_idx:], 
                                                self.events['p'][self.last_idx:])):
                t, x, y, pol = float(ti/1e6), int(xi), int(yi), int(pi)
                event_list.append([t, x, y, pol])
                if self.last_stamp is None:
                    self.last_stamp = t
                if t > self.last_stamp + self.duration_s:
